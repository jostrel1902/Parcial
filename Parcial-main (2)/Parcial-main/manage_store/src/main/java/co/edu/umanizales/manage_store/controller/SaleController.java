...
public class SaleController {
    ...
    public ResponseEntity<ResponseDTO> getStoreByQuantity(int cant,String code) {
        int findSale = saleService.getTotalSalesBySeller(code);
        while (findSale != 0) {
            return new ResponseEntity<>(new ResponseDTO(200,saleService.getTotalSalesBySeller(code)>cant,
                    null),HttpStatus.OK);

        }
        return new ResponseEntity<>(new ResponseDTO(409, "no hay vendedores con esas ventas mayores ", null),
                HttpStatus.BAD_REQUEST);
    }
}
